...
void loop() {
   //Read the value of the pot and map it from 10 to 10.000 us. AC fre quency is 50Hz, so period is 20ms. We want to control the power
   //of each half period, so the maximum is 10ms or 10.000us. In my case I've maped it up to 7.200us since 10.000 was too much
   
   //valor = map(analogRead(A4),0,1024,7200,10);
   valor = map(analogRead(A4),0,1024,9500,0);
   //
...
ISR(PCINT0_vect){
 // digitalWrite(13,HIGH);
  ...
  if(PINB & B00000001){                               //We make an AND with the pin state register, We verify if pin 8 is HIGH???
    ...
    detectado=1;                                      //We have detected a state change!
